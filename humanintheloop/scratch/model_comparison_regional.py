...
def parse_args(argv=None):
    ...
    parser.add_argument("--lat-min", type=float, help="Minimum latitude for station filtering.")
    parser.add_argument("--lat-max", type=float, help="Maximum latitude for station filtering.")
    parser.add_argument("--lon-min", type=float, help="Minimum longitude for station filtering.")
    parser.add_argument("--lon-max", type=float, help="Maximum longitude for station filtering.")
...
def run_evaluation(
    ...
    if lat_min is not None and lat_max is not None:
        stations = [s for s in stations if s.get("latitude") and lat_min <= s["latitude"] <= lat_max]
    if lon_min is not None and lon_max is not None:
        stations = [s for s in stations if s.get("longitude") and lon_min <= s["longitude"] <= lon_max]
    ...
    if not stations:
        return {
            ...
            "message": f"No observation stations matched the location/spatial query."
        }
