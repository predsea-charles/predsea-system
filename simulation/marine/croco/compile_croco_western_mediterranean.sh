#!/usr/bin/env bash
# compile_croco_western_mediterranean.sh - Compile custom CROCO binary for the Western Mediterranean 1 km Grid.
set -euo pipefail
...
# Run the unified, robust cross-platform Python patching utility. These are the
# interior dimensions corresponding to the 2221 x 1056 rho-point grid.
PREDSEA_CROCO_LM=2219 \
PREDSEA_CROCO_MM=1054 \
PREDSEA_CROCO_N=32 \
PREDSEA_CROCO_NP_XI=16 \
PREDSEA_CROCO_NP_ETA=12 \
python3 ../../simulation/marine/croco/patch_croco_source.py

# 4. Invoke jobcomp
echo "🏗️ Compiling CROCO Western Mediterranean regional binary..."
# We pass source path and enable jobs parallelization
./jobcomp --src croco_src/OCEAN --jobs 4

# Check output executable
if [ -f "croco" ]; then
    cp croco ../../simulation/marine/croco/croco_western_mediterranean.exe
    echo "========================================================================="
    echo "✅ CROCO Western Mediterranean compiled successfully!"
    echo "   Binary saved: simulation/marine/croco/croco_western_mediterranean.exe"
    echo "========================================================================="
else
