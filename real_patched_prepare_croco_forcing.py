...
def depth_average_velocity(velocity: np.ndarray, z_w: np.ndarray) -> np.ndarray:
    ...
    if np.any(layer_thickness <= 0.0):
        bad = np.where(np.any(layer_thickness <= 0.0, axis=0))
        ...
        print(
            ...
            f"h_u/h_v=not_in_scope mask=mask not in scope",
            flush=True,
        )
...
def main() -> int:
    ...
    for t in range(nt):
        ...
        if t == 0:
            print(
                f"⚠️ DIAG Zeta Anomaly Check at j=367, i=245:\n"
                f"  zeta_v[0][367, 245] = {zeta_v[0, 367, 245]:.6f}\n"
                f"  zeta_clm[0][367, 245] = {zeta_clm[0, 367, 245]:.6f}\n"
                f"  zeta_clm[0][368, 245] = {zeta_clm[0, 368, 245]:.6f}\n"
                f"  mask_rho[367, 245] = {mask_rho[367, 245]}\n"
                f"  mask_rho[368, 245] = {mask_rho[368, 245]}\n"
                f"  lon_rho[367, 245] = {lon_rho[367, 245]:.6f}, lat_rho[367, 245] = {lat_rho[367, 245]:.6f}\n"
                f"  h_v[367, 245] = {h_v[367, 245]:.6f}, h[367, 245] = {h[367, 245]:.6f}, h[368, 245] = {h[368, 245]:.6f}",
                flush=True,
            )
