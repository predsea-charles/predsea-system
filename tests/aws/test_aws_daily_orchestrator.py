...
def test_native_marine_staging_uses_validated_region_scoped_products(monkeypatch):
    def fake_run(command, *, dry_run=False):
        ...
        for filename in daily.NATIVE_MARINE_FILES:
    ...
    assert len(s3.uploads) == len(daily.MARINE_REGIONS) * 6
    keys = {upload[2] for upload in s3.uploads}
    assert "forcing/cmems/2026-08-23/cmems_croco_currents_3d_alboran_1km.nc" in keys
