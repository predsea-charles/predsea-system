...
def depth_average_velocity(velocity: np.ndarray, z_w: np.ndarray) -> np.ndarray:
    ...
    z_w_arr = np.asarray(z_w, dtype=float)
    layer_thickness = np.diff(z_w_arr, axis=0)
    if velocity.shape != layer_thickness.shape:
        raise ValueError("velocity and CROCO layer-thickness shapes do not agree")
    
    bad = np.where(np.any(layer_thickness <= 0, axis=0))
    if bad[0].size:
        j0, i0 = int(bad[0][0]), int(bad[1][0])
        print(
            f"⚠️ DIAG non-positive thickness: {bad[0].size} cols. "
            f"j={j0} i={i0} z_w_col={z_w_arr[:, j0, i0].round(3).tolist()} "
            f"h_u/h_v=not_in_scope mask=mask not in scope zeta=not_in_scope",
            flush=True,
        )
...
def verify_transport_consistency(
    ...
    diff_u = np.abs(ubar - ubar_reconstructed)
    diff_v = np.abs(vbar - vbar_reconstructed)
    max_diff_u = float(np.max(diff_u))
    max_diff_v = float(np.max(diff_v))

    if max_diff_u > tolerance or max_diff_v > tolerance:
        idx_u = np.unravel_index(np.argmax(diff_u), diff_u.shape)
        idx_v = np.unravel_index(np.argmax(diff_v), diff_v.shape)
        ju, iu = int(idx_u[-2]), int(idx_u[-1])
        jv, iv = int(idx_v[-2]), int(idx_v[-1])
        z_w_u_col = z_w_u[:, ju, iu].round(3).tolist()
        z_w_v_col = z_w_v[:, jv, iv].round(3).tolist()
        print(
            f"⚠️ DIAG Barotropic transport mismatch: max_diff_u={max_diff_u:.6e} at j={ju},i={iu} (ubar={ubar[...,ju,iu]}, u_rec={ubar_reconstructed[...,ju,iu]}), "
            f"max_diff_v={max_diff_v:.6e} at j={jv},i={iv} (vbar={vbar[...,jv,iv]}, v_rec={vbar_reconstructed[...,jv,iv]}). "
            f"z_w_u_col={z_w_u_col} z_w_v_col={z_w_v_col} mask=mask not in scope",
            flush=True,
        )
        raise ValueError(
