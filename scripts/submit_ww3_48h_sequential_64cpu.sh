...
# PredSea WW3 48h Forecast Sequential 64-vCPU Submission Script
#
# Sequential 64-vCPU Execution Strategy:
# Runs each region one after another, giving 100% of the project's 64-vCPU quota
# to each regional solver for maximum execution speed and stability.

RUN_DATE="${1:-$(date -u +%Y-%m-%d)}"
GCS_FORCING_BASE="gs://predsea-daily-outputs-test/scratch/ww3-48h-forcing"
GCS_GRID_BASE="gs://predsea-daily-outputs-test/scratch/ww3-smoke-test"
GCS_OUTPUT_BASE="gs://predsea-daily-outputs-test/predictions/${RUN_DATE}/runs/ww3-48h-forecast"
...
echo "========================================================================"
echo "🚀 Launching Sequential 64-vCPU WW3 48h Forecast Test"
...
for region in "${REGIONS[@]}"; do
  job_id_safe="ww3-48h-${region//_/-}-64cpu-${TIMESTAMP}"
  echo ""
